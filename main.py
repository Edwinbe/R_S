...
def parse_global_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument('--embedding_gen', type=int, default=0)
    parser.add_argument('--inference_config', type=str, default="train")
...
if __name__ == '__main__':
    ...
    if args.train == 0:
    ...
    elif args.embedding_gen == 1:
        global embedding_generation
        ...
        model.load_model(model_path=args.test_path)
        test_result, _ = runner.evaluate(model, inference_config)
    else:
        runner.train(model)
