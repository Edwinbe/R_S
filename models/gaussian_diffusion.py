...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample(self, model, x_start, v, steps, sampling_noise=False):
        ...
        for i in indices:
            t = th.tensor([i] * x_t.shape[0]).to(x_start.device)
            out = self.p_mean_variance(model, x_t, v,t)
            if sampling_noise:
    ...
    def training_losses(self, model, x_start, v,reweight=False):
    ...
    def p_mean_variance(self, model, x, v,t):
        """
