...
class BaseRunner(object):
    ...
    @staticmethod
    @torch.no_grad()
    def predict(model: BaseModel, test_loader: DataLoader,query_gen=None):
        model.eval()
        predictions = list()
...
#logging
    ...
    def evaluate_diff(self, model: BaseModel, mode: str, diffusion, transformer,sampling_steps,sampling_noise):
        model.eval()
        transformer.eval()
        predictions = list()
        if mode == 'val':
            rec_loader = self.rec_val_loader            
            src_loader = self.src_val_loader        
        elif mode == 'test':
            rec_loader = self.rec_test_loader
            src_loader = self.src_test_loader
        else:
            raise ValueError('test set error')
        start = time.time()
        for step, batch in enumerate(rec_loader):
            rec_fusion, src_fusion, query_emb = self.get_batch_emb(model,batch)
            rec_fusion = rec_fusion.to(model.device)
            src_fusion = src_fusion.to(model.device)
            query_emb = query_emb.to(model.device)
            query_gen = diffusion.p_sample(transformer, query_emb, rec_fusion, sampling_steps, sampling_noise)
            query_gen = query_gen.to(model.device)
            # batch = batch.to(model.device)
            prediction = model.predict(batch,query_gen) #==>inputs
            predictions.extend(prediction.cpu().data.numpy())
        rec_predictions = np.array(predictions)
        src_predictions = self.predict(model, src_loader)
    ...
    def get_batch_emb(self,model: BaseModel, batch):
        model.eval()
        rec_fusion, src_fusion,query_emb = model.get_embedding(utils.batch_to_gpu(batch, model.device))
